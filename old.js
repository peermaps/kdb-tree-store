...
KDB.prototype._pointOverflow = function (buf, n, depth, regions, pt, cb) {
  ...
  if (self._addRegions(lr[0], [[left,exleft], [right,exright]])) {
    ...
    //var pivot = self._median(regions[regions.length-1][0], sp.pivot)
    var r = regions.length - 1
    self._regionOverflow(regions[r][0], regions[r][1], sp.pivot, d,
    function (err, rs) {
      if (err) return cb(err)
      if (self._addRegions(buf, rs)) {
        var done = ndone(2, cb)
        self.store.put()
        self.store.put()
      } else {
...
KDB.prototype._regionOverflow = function (buf, n, pivot, axis, cb) {
  cb = once(cb || noop)
  var self = this
  var len = self.types.length

  var sp = self._splitRegionPage(buf, axis, pivot)
  if (sp.center.length === 0) return ondone()
  var done = ndone(sp.center.length, ondone)
  throw new Error('todo: center overflow')
  sp.center.forEach(function (c) {
    var cn = c[0], cr = c[1]
    self.store.get(cn, function (err, buf) {
console.log('GET', cn, err, buf) 
      if (err) cb(err)
      else if (buf[0] === POINT) {
        console.log('POINT')
        throw new Error('point overflow')
        self._pointOverflow()
      } else if (buf[0] === REGION) {
        console.log('REGION')
        throw new Error('region overflow')
        self._regionOverflow(
          regions.slice(0,-1), buf, cn,
          pivot, axis, axis, cb
        )
      } else cb(new Error('unexpected page type: ' + buf[0]))
    })
  })
  //throw new Error('region overflow!')
  function ondone (err) {
