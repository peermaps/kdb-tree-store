...
test('index', function (t) {
  ...
  for (var i = 0; i < n; i++) (function () {
    ...
    kdb.insert([x,y,z], loc, function (err) {
      ...
      kdb.query([x,y,z], { index: true }, function (err, pts) {
        ...
        if (pts[0]) {
          ...
          t.equal(pts[0].index.length, 2, 'index property')
  ...
  function check () {
    kdb.query([[15,50],[-60,10],[50,100]], { index: true }, onquery)
    ...
    function onquery (err, pts) {
      ...
      for (var i = 0; i < Math.max(pts.length, expected.length); i++) {
        ...
        t.equal(pts[i].index.length, 2, 'index property')
      }
      t.end()
    }
  }
